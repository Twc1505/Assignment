...
// Query to get quizzes the user has attempted
$quiz_sql = "
    SELECT DISTINCT quiz.quizId, quiz.title, quiz.description, user.username
    FROM attempt 
    INNER JOIN quiz ON attempt.quizId = quiz.quizId
    LEFT JOIN user ON user.userId = quiz.userId
    WHERE attempt.userId = ? $time_filter
    ORDER BY $sort $order
    LIMIT ? OFFSET ?";
...
$count_sql = "SELECT COUNT(DISTINCT quiz.quizId) AS total 
              FROM attempt 
              INNER JOIN quiz ON attempt.quizId = quiz.quizId
              WHERE attempt.userId = ? $time_filter";
...
<head>
    ...
    <title>Previously Used Quizzes</title>
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <ul class="menu">
            <li class="menu-item"><a href="LibraryPublished.php">Published contents</a></li>
            <li class="menu-item active"><a href="PreviouslyUsed.php">Previously used</a></li>
        </ul>
    </aside>

    <!-- Content Section -->
    <section class="content">
        <h2><span class="person-icon"><i class="fas fa-user"></i></span> Previously Used Quizzes</h2>
        ...
        <div class="quiz-cards">
            <?php if (empty($quiz_data)) { ?>
                <p>No previously used quizzes are available.</p>
            <?php } else { ?>
                <?php foreach ($quiz_data as $quiz) { ?>
                    <div class="quiz-card">
                        <!-- Clickable link for the card -->
                        <a href="../explore/quiz_details.php?quizID=<?= htmlspecialchars($quiz['quizId']) ?>" 
